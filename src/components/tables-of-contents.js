...
function useActiveId(itemIds) {
    ...
    const renderItems = (items, activeId) => (
        <ol>
            {items.map((item) => (
                <li key={item.url}>
                    <a href={item.url} style={{color: activeId === item.url.slice(1) ? "#0EFB69" : "#FC4056"}}>{item.title}</a>
...
const TableOfContents = (props) => {
    ...
    return (
    <details className={hide ? 'table-of-contents hide-out' : 'table-of-contents'} open>
